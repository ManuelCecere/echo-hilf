sglang_base_url = "http://0.0.0.0:8000/v1"
sglang_api_key = "EMPTY"  # Assuming no API key is required for your local server
...
class SglangOpenAI(LLM):
    ...
    def _get_client(self) -> SyncOpenAI:
        # Always return a new client pointing to the Sglang server, respecting the `reuse_client` setting.
        if not self.reuse_client or self._client is None:
    ...
    def _get_aclient(self) -> AsyncOpenAI:
        # Always return a new async client pointing to the Sglang server, respecting the `reuse_client` setting.
        if not self.reuse_client or self._aclient is None:
...
def load_llm(max_tokens):
    return SglangOpenAI(max_tokens=max_tokens)
